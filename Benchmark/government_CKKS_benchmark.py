...
if os.path.isfile('./privatekey_context_CKKS_benchmark'):
    file = open("privatekey_context_CKKS_benchmark","r")
    hex_line = file.read()
    file.close()
...
# Get private key context for vote tabulation + decryption
if os.path.isfile('./tabulation_result_CKKS_benchmark'):
    file = open("tabulation_result_CKKS_benchmark","r")
    hex_line = file.read()
    file.close()
